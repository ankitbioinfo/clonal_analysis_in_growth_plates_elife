...
def plot_everything(datapath,ax,mycolor,legendlabel,alpha,lw):
	...
	ax[0,0].hist(AZ,bins=30,density=True,histtype='step', color=mycolor, facecolor=mycolor,alpha=1,label=legendlabel)
	...
	x=data[:,3]
	n,bins,_=ax[0,1].hist(x,bins=30,density=True,histtype='step', color=mycolor,facecolor=mycolor,alpha=1)
	...
	EL=data[:,2]*factor
	ax[1,0].hist(EL,bins=30,density=True,histtype='step',color=mycolor, facecolor=mycolor,alpha=1,label=legendlabel)
	...
	hist=ax[1,1].hist(deg,bins=bins,density=True,histtype='step', color=mycolor,linewidth=lw,facecolor=mycolor,alpha=alpha)
	x = [hist[0], 0.5*(hist[1][1:]+hist[1][:-1])]
	xdata=x[1]
	ydata=x[0]
	...
	#popt, pcov = curve_fit(wigner, np.log(xdata), np.log(ydata))
	#legend4=':%5.3f  ' % tuple(popt)
	#ax[1,1].plot(xdata, wigner(xdata, *popt), mycolor+'-',label=legendlabel+legend4)
...
def main():
	...
	datapath2='plothist_embryo_random/DF/'


	fig,ax=plt.subplots(2,2,figsize=(8,5))
	
	plot_everything(datapath1,ax,'r','E18.5 DF real',0.3,5)
	plot_everything(datapath2,ax,'b','E18.5 DF random',0.6,1)
	...
	fig.savefig('histogram_elevation_random_DF.png',bbox_inches='tight',dpi=300)
	fig.clf()
