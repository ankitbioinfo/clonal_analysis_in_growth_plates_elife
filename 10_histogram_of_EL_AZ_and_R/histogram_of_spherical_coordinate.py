...
ax[0,0].hist(AZ,bins=50,density=True,facecolor='g',alpha=0.75)
...
x=data[:,3]
n,bins,_=ax[0,1].hist(x,bins=50,density=True,facecolor='g',alpha=0.75)
...
ax[0,1].plot(bins, best_fit_line,'k-')
...
EL=data[:,2]*factor
ax[1,0].hist(EL,bins=50,density=True,facecolor='g',alpha=0.75)
...
hist=ax[1,1].hist(deg,bins=bins,density=True,facecolor='g',alpha=0.75)
x = [hist[0], 0.5*(hist[1][1:]+hist[1][:-1])]
xdata=x[1]
ydata=x[0]
popt, pcov = curve_fit(exponential, xdata, ydata)
ax[1,1].plot(xdata, exponential(xdata, *popt), 'k-')
...
fig.savefig('Mature_manual_cluster.png',bbox_inches='tight',dpi=300)
fig.clf()
