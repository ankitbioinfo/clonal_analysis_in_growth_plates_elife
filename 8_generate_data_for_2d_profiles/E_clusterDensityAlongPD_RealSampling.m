...
%cluster properties calculate 
 for j=1:length(LCC)
     ...
     [ center, radii, evecs, ~, ~ ] = ellipsoid_fit_new( ClusterShape );
     combinedCellVolumeInCluster(j,1)=mergeVol;
     [K,V]=convhull(ClusterShape); outerSurfacePoints=ClusterShape(unique(K(:)),:);
     
     
%     plot3(ClusterShape(:,1),ClusterShape(:,2),ClusterShape(:,3),'r.');
%     hold on 
%     
...
%     [sa1,sb1]=min(height); [sa2,sb2]=max(height);
    
     
         
     boundingboxVolume=  prod(max(ClusterShape)-min(ClusterShape));
...
%      end
      ...
      height=ClusterShape(:,3);
      colheight(j,:)=max(height)-min(height);

     normalized_volume(j,1)=V/ min (boundingboxVolume, fittedEllipsoidVolume )  ;
     just_volume(j,1)=V;
     just_SA(j,1)=CalculateSurfaceArea(K,ClusterShape);
     normalized_surface_area(j,1)=just_SA(j,1)/ellipsoid_surface_area(radii);
     
     VolumeFraction(j,1)=  mergeVol./V;
    

     sphericity(j,1)= (pi^(1/3))*((6*V).^(2/3)) ./ just_SA(j,1);
     clusterSize(j,1)=length(LCC{j});
     
     allometric(j,1)=(V^2/3)/just_SA(j,1);
     
     [clusterPCvector,~,latent]=pca(ClusterShape);
     cluster_mask_fitted_ellipsoid_Rg(j,1)= sqrt(sum(radii.^2)/5);
          
     just_rg(j,1)=radiusOfGyration(c,LCC{j});
