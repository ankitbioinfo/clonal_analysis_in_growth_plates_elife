...
dir2=strcat('ClusterFeatures_profiles_ProximalTibia','/');
if ~exist([dir2],'dir')
    mkdir([dir2]);
end
...
for fi=[2,4,6]
    ...
    dataFile1=strcat('micron15/MakeListNuclei_All/',s{2});
...
for fi=[2,4,6]
    path=allpath{fi};s=strsplit(path,'Nuclei_and_CellsE185_'); 
    dataFile2=strcat('micron15/merge_clones/',s{2});
...
legname={'expected columns','merged clones'};
mycolor={'ro-','bs-','g*-'};
...
%     xlabel('Long axis of bone')
    ...
    if (k==26)|(k==27)|(k==28)|(k==37)|(k==35)|(k==29)|(k==30)|(k==31)|(k==32)|(k==22)|(k==23)|(k==24)|(k==25)
        Feature.savefile=0;
    else
        Feature.savefile=1;
