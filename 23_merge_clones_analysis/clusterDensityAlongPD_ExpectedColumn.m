...
[~,LCC]=readClusterFile([inputpath,'ExpectColumnLikeCluster.dat']); 
...
%    no_of_nodes=length(unique(LCC{j}));
 ...
 for i=1:length(AllEdgesOFClusterSaved)
     ...
     for j=1:size(E,1)
         fprintf(fid,'%d\t%0.3f\t%0.3f\t%0.3f\t%d\t%d\n',i,E(j,1),E(j,2),E(j,3),E(j,4),E(j,5));
     end
 end
 fclose(fid);
...
%             sph=[az,el,r];
            ...
            for i=1:size(E,1)
                ...
                c=c-mean(c);
                [az,el,r]=cart2sph(c(:,1),c(:,2),c(:,3));
                sph(count:count+1,:)=[az,el,r,nodeid(E(i,[1,2]))',nodeid(E(i,[2,1]))'];
                count=count+2;
            end    
