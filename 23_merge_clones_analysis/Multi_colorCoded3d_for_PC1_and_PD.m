...
for gi=1%2:length(allpath)
        ...
        inputpath=strcat(dpath,'MakeListNucleiLabelled/',s{2});
        outputpath_mask=strcat(dpath,'MakeListClustersMask/',s{2});
        outputpath=strcat('micron15/merge_clones/',s{2});
...
[merged_mask_id,temp]=readClusterFile(['micron15/merge_clones/',s{2},'/ClusterMaskMerged.dat']);
...
bininterval=[0,20,40,50,60,70,80,92];
mycolorAll={'m','y','c','g',orange,'r','b'};

bininterval=[0,15,30,45,60,70,80,92]; %First 
mycolorAll={'m','y','c','g',orange,'r','b'}; %First 

% bininterval=[0,15,30,45,60,70,80,92]; %Second 
% mycolorAll={'r',orange,'y','g','m','c','b'}; %Second 
% 
% bininterval=[0,15,30,45,60,70,80,92]; %third 
% mycolorAll={'b','c','m','g','y',orange,'r'}; %third  
...
%      end
      ...
      ClusterShape=[];
      maskid=sarah_mask_id(merged_mask_id{j});
     
      for k=1:length(maskid)
          ClusterShape=[ClusterShape; sarah_mask.nuc{maskid(k)}];
      end
...
%     end
    ...
    factor=1; quiver3(mu(1),mu(2),mu(3),factor*vec(1,3),factor*vec(2,3),factor*vec(3,3),d(3),'color',mycolor,'LineWidth',2); %PC1
...
set(gca,'Color','k');
 % saveas(h,[savepath,'colorcoded',fname,'.png']);
  saveas(h,[savepath,'mergeclones_multicolor/sec_',num2str(section(sec)),'_colormap_between_PD_and_PC1_',fname,'.fig']);
  dlmwrite([savepath,'mergeclones_multicolor/',fname,'.dat'],data);
  close all 
end

length(data)
100*sum(data(:,1)>60)/length(data)
