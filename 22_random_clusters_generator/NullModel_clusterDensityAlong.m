...
for realization=1:2
    ...
    [colcludata,coldata,cludata]=  FindAllFeatures(LCC,rand_dir_output,nuc,c,celvolume,GVec,RZ,HZ,realization,radius);
    
    Xbin=colcludata{1};Ybin_iter=colcludata{2}; AvgMeanFeatures_iter=colcludata{3} ; AvgStdFeatures_iter=colcludata{4} ;
    colclu_realization_Ybin(:,realization)=Ybin_iter;    
    for i=1:length(AvgMeanFeatures_iter)
        colclu_realization_AvgMeanFeatures{i}(:,realization)=AvgMeanFeatures_iter{i};
    end
    
    Xbin=coldata{1};Ybin_iter=coldata{2}; AvgMeanFeatures_iter=coldata{3} ; AvgStdFeatures_iter=coldata{4} ;
    col_realization_Ybin(:,realization)=Ybin_iter;    
    for i=1:length(AvgMeanFeatures_iter)
        col_realization_AvgMeanFeatures{i}(:,realization)=AvgMeanFeatures_iter{i};
    end
    
    Xbin=cludata{1};Ybin_iter=cludata{2}; AvgMeanFeatures_iter=cludata{3} ; AvgStdFeatures_iter=cludata{4} ;
    clu_realization_Ybin(:,realization)=Ybin_iter;    
    for i=1:length(AvgMeanFeatures_iter)
        clu_realization_AvgMeanFeatures{i}(:,realization)=AvgMeanFeatures_iter{i};
    end
    
end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
realization_Ybin=colclu_realization_Ybin; 
realization_AvgMeanFeatures=colclu_realization_AvgMeanFeatures;
Xbin=colcludata{1};
...
save([rand_dir_output,'SaveVariablesForPlot.mat'], 'AvgMeanFeatures','AvgStdFeatures','Xbin','Ybin','YbinStd','realization_Ybin','realization_AvgMeanFeatures');

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
realization_Ybin=col_realization_Ybin; 
realization_AvgMeanFeatures=col_realization_AvgMeanFeatures;
Xbin=coldata{1};
...
save([rand_dir_output,'SaveVariablesForPlotCol.mat'], 'AvgMeanFeatures','AvgStdFeatures','Xbin','Ybin','YbinStd','realization_Ybin','realization_AvgMeanFeatures');


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
realization_Ybin=clu_realization_Ybin; 
realization_AvgMeanFeatures=clu_realization_AvgMeanFeatures;
Xbin=cludata{1};
...
save([rand_dir_output,'SaveVariablesForPlotClu.mat'], 'AvgMeanFeatures','AvgStdFeatures','Xbin','Ybin','YbinStd','realization_Ybin','realization_AvgMeanFeatures');
...
function [colcludata,coldata,cludata]=FindAllFeatures(LCC,rand_dir_output,nuc,c,celvolume,GVec,RZ,HZ,realization,radius)
BoneGrowthAxis=[0; 0 ; 1]
fid=fopen([rand_dir_output,'degree_of_clusters',num2str(realization),'.txt'],'w');
...
%cluster properties calculate 
 for j=1:length(LCC)
     ...
     combinedCellVolumeInCluster(j,1)=mergeVol;
      ...
      randomedges=load(strcat(rand_dir_output,'Realization/Iteration_',num2str(realization),'/Edges_',num2str(j),'.dat'));
...
%      end
     ...
     [K,mskvol]=convhull(mskc);  outerSurfacePoints=mskc(unique(K(:)),:);
     ...
     AngleBetweenClusterPC1AndBone_PD(j,1)=90-180/pi*oangle(clusterPCvector(:,1),BoneGrowthAxis);
     AngleBetweenClusterPC2AndBone_PD(j,1)=90-180/pi*oangle(clusterPCvector(:,2),BoneGrowthAxis);
     AngleBetweenClusterPC3AndBone_PD(j,1)=90-180/pi*oangle(clusterPCvector(:,3),BoneGrowthAxis);
     ...
     LocalBiaxial(j,:)=mean(biaxial,1);
        ...
        %[j,length(randomedges),length(edges)]
 ...
 fid=fopen([rand_dir_output,'spherical_coordinate',num2str(realization),'.txt'],'w');
...
clusterPC3_by_PC2=clusterPC3./clusterPC2;
 ...
 clusterSize,largestDistance,AngleBetweenClusterPC1AndBone_PD,AngleBetweenClusterPC2AndBone_PD,AngleBetweenClusterPC3AndBone_PD); %angleMeasurement,
 filename = [rand_dir_output,'nuclei_column_stats_data',num2str(realization),'.xlsx'];
...
save([rand_dir_output,'AllFeaturesSave',num2str(realization),'.mat'],'normalized_volume','normalized_surface_area','clusterSize','sphericity','clusterPC1', 'clusterPC2', 'clusterPC3',...
'clusterPC2_by_PC1','clusterPC3_by_PC1','clusterPC3_by_PC2','just_SA','just_rg',...
    ...
    'CluAvgDeg','smallest_mode','cluster_mask_fitted_ellipsoid_Rg','allometric', 'averageStepSize','LCC','LocalBiaxial',...
'AngleBetweenClusterPC1AndBone_PD','AngleBetweenClusterPC2AndBone_PD','AngleBetweenClusterPC3AndBone_PD','just_volume','colheight');
...
Features={normalized_volume,just_volume,normalized_surface_area,just_SA,clusterSize,sphericity,clusterPC1, clusterPC2, clusterPC3,clusterPC2_by_PC1,clusterPC3_by_PC1,clusterPC3_by_PC2,...
    ...
    CluAvgDeg,diameter,allometric,averageStepSize,no_ofLoops,colheight,AngleBetweenClusterPC2AndBone_PD,AngleBetweenClusterPC3AndBone_PD};


%radius=clusterPositionAndRadius;
LCC_col=find(AngleBetweenClusterPC1AndBone_PD>60);  radius_col=radius(LCC_col);
LCC_clu=find(AngleBetweenClusterPC1AndBone_PD<=60); radius_clu=radius(LCC_clu);
LCC_all=1:length(LCC);
%[mean(AngleBetweenClusterPC1AndBone_PD),mean(AngleBetweenClusterPC1AndBone_PD(LCC_col)),  mean(AngleBetweenClusterPC1AndBone_PD(LCC_clu))]

[AvgMeanFeatures,AvgStdFeatures,Xbin,Ybin,dataAvgMeanFeatures]=save_data_for_create_profile(LCC_all,Features,RZ,HZ,radius,21);
colcludata={Xbin,Ybin, AvgMeanFeatures,AvgStdFeatures};
%nanmean(AvgMeanFeatures{33})
%save([outputpath,'SaveVariablesForPlot_.mat'],   'AvgMeanFeatures','AvgStdFeatures','Xbin','Ybin','dataAvgMeanFeatures');

[AvgMeanFeatures,AvgStdFeatures,Xbin,Ybin,dataAvgMeanFeatures]=save_data_for_create_profile(LCC_col,Features,RZ,HZ,radius_col,21);
coldata={Xbin,Ybin, AvgMeanFeatures,AvgStdFeatures};
%save([outputpath,'SaveVariablesForPlotCol_.mat'],   'AvgMeanFeatures','AvgStdFeatures','Xbin','Ybin','dataAvgMeanFeatures');
%nanmean(AvgMeanFeatures{33})

[AvgMeanFeatures,AvgStdFeatures,Xbin,Ybin,dataAvgMeanFeatures]=save_data_for_create_profile(LCC_clu,Features,RZ,HZ,radius_clu,21);
cludata={Xbin,Ybin, AvgMeanFeatures,AvgStdFeatures};
%save([outputpath,'SaveVariablesForPlotClu_.mat'],   'AvgMeanFeatures','AvgStdFeatures','Xbin','Ybin','dataAvgMeanFeatures');
%nanmean(AvgMeanFeatures{33})
...
function [AvgMeanFeatures,AvgStdFeatures,Xbin,Ybin,dataAvgMeanFeatures]=save_data_for_create_profile(LCC,Features,RZ,HZ,radius,binsize)
    ...
    for i=1:length(zone)-1
        cellsIdInXInterval=[];   
          for j=1:length(LCC)
               if ((radius(j,1) >Xbin(i)) &  (radius(j,1) <=Xbin(i+1)))
                     Ybin(i)=Ybin(i)+1;
                     cellsIdInXInterval=[cellsIdInXInterval,LCC(j)];
               end
          end
