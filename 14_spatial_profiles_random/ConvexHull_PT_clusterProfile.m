...
legname={'PT E18.5 cluster labeled','PT E18.5 cluster unlabeled'};
...
savename={'Volume_normalized','Volume','SurfaceArea_normalized','SurfaceArea','clusterSize','clusterSphericity',  ...
          ...
          'biaxial_S','biaxial_P','biaxial_D','biaxial_C',  'AngleBetweenClusterPC1AndBone_PD' ,'coordination_number','diameter','allometric', 'randomWalktheory',...
          'no_ofloops','columnHeight','AngleBetweenClusterPC2AndBone_PD','AngleBetweenClusterPC3AndBone_PD'};
  ...
  '<cluster PC1>','<cluster PC2>','<cluster PC3>','<cluster PC2/PC1>','<cluster PC3/PC1>','<cluster PC3/PC2>', '$\langle\overline{R_g}\rangle$','<R_g>',...
   ...
   '<Global OOP 1>','<Global OOP 2>','<Global OOP 3>',  '<angle(plane, P-D axis)>','<angle(plane,PC1)>',...
            '<angle(plane, PC2)>', '<angle(plane,PC3)>', '<largest eigenvalue of Hessian>', '<smallest eigenvalue of Hessian>',...
             '<R_g>', '< OOP S>', '< OOP P>', '< OOP D>', '< OOP C>', '<\alpha>','$\langle{avg\;deg}\rangle$','<diameter>','Vol^{2/3}/SA','<R_g>',...
             '<# of cycles>','<height>','angle PC2-PD', 'angle PC3-PD'};
...
dir2=strcat('Random_spatial_profile_PT','/');
if ~exist([dir2],'dir')
    mkdir([dir2]);
end
...
path=allpath{2};s=strsplit(path,'Nuclei_and_CellsE185_'); p11=strcat(dpath,'RealSampling/',s{2});r11=strcat(dpath,'RandomSampling/',s{2});
path=allpath{4};s=strsplit(path,'Nuclei_and_CellsE185_'); p12=strcat(dpath,'RealSampling/',s{2});r12=strcat(dpath,'RandomSampling/',s{2});
path=allpath{6};s=strsplit(path,'Nuclei_and_CellsE185_'); p13=strcat(dpath,'RealSampling/',s{2});r13=strcat(dpath,'RandomSampling/',s{2});


temp1=load([p11,'SaveVariablesForPlotClu_.mat']);
temp2=load([p12,'SaveVariablesForPlotClu_.mat']);
temp3=load([p13,'SaveVariablesForPlotClu_.mat']);
dataReal={temp1,temp2,temp3};

temp1=load([r11,'SaveVariablesForPlotClu.mat']);
temp2=load([r12,'SaveVariablesForPlotClu.mat']);
temp3=load([r13,'SaveVariablesForPlotClu.mat']);
