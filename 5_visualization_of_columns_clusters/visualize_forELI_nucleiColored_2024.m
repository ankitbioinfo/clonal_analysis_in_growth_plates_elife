...
outputVisualize='visualize_clusters_with_PC1_and_avg_EL_2024/';
...
[LCC,LCC1]=readClusterFile([outputpath,'Cluster.dat']);
colmaskid=load([outputpath,'ClusterMask.dat']);
...
for j=1:size(connectEdg,1)
    E=connectEdg(j,:);
    plot3(a.centroid(E,1), a.centroid(E,2), a.centroid(E,3), 'K-','linewidth',1);
    hold on 
end
...
plot3(c(:,1),c(:,2),c(:,3),'k.','markersize',0.75);
    ...
    %ovec=vec;
    %oval=val;
    mu=mean(c); d=sqrt(latent);vec=clusterPCvector;
    %d = sqrt(diag(val));
    hold on;
...
%     set( p, 'FaceColor', 'k', 'EdgeColor', 'none' ); alpha(0.1);
%     camlight;
%      lighting phong;       
