...
dir2=strcat('Profiles_DF_E185_col_clu','/');
if ~exist([dir2],'dir')
    mkdir([dir2]);
end
...
for fi=[1,3,5]
    ...
    d_femur{count}=load([dataFile1,'SaveVariablesForPlotCol_.mat']);
    count=count+1;
end
...
for fi=[1,3,5]
    path=allpathEmbryo{fi};s=strsplit(path,'Nuclei_and_CellsE185_'); 
    dataFile2=strcat('../../EmbryoData/Femur_Tibia/MakeListNucleiLabelled/',s{2});
    d_tibia{count}=load([dataFile2,'SaveVariablesForPlotClu_.mat']);
    count=count+1;
end
...
legname={'E18.5 DF Col','E18.5 DF Clu'};
mycolor={'ro-','bs-','g*-'};
