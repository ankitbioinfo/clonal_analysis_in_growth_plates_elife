...
for gi=6%2:length(allpath)
        ...
        outputpath=strcat('../Femur_Tibia/MakeListNucleiLabelled/',s{2});
        outputpath_mask=strcat('../Femur_Tibia/MakeListClustersMask/',s{2});
...
%set(gca,'Color','k');
 % saveas(h,[savepath,'colorcoded',fname,'.png']);
  saveas(h,[savepath,'2colorCoded3d_for_PC1_and_PD/sec_',num2str(section(sec)),'_colormap_between_PD_and_PC1_',fname,'.fig']);
  dlmwrite([savepath,'2colorCoded3d_for_PC1_and_PD/',fname,'.dat'],data);
  close all 
end
