...
%         
%     
ylabelname={'$\langle\overline{volume}\rangle$', '<volume>',  '$\langle\overline{surface\; area}\rangle$',  '<surface area>',  '<size>', '<sphericity>',...
  '<PC1>','<PC2>','<PC3>','<PC2/PC1>','<PC3/PC1>','<PC3/PC2>', '$\langle\overline{R_g}\rangle$','<R_g>',...
   '<volume fraction>', '<Local OOP 1>','<Local OOP 2>','Local <OOP 3>',...
   '<Global OOP 1>','<Global OOP 2>','<Global OOP 3>',  '<angle(plane, P-D axis)>','<angle(plane,PC1)>',...
            '<angle(plane, PC2)>', '<angle(plane,PC3)>', '<largest eigenvalue of Hessian>', '<smallest eigenvalue of Hessian>',...
...
dir2=strcat('ML_Profiles_DF_P40_col_clu','/');
if ~exist([dir2],'dir')
    mkdir([dir2]);
end
...
allpathPostnatal={
'processedData/S151_m2_distalfemur/',
'processedData/S151_m2_proximaltibia/',
'processedData/S152_m3_distalfemur/',
'processedData/S152_m3_proximaltibia/',
'processedData/S152_m4_distalfemur/',
'processedData/S152_m4_proximaltibia/',
}; 
...
for fi=[1,3,5]
    ...
    %dataFile1=strcat('../proximalTibia_DistalFemur/MakeListNucleiLabelled/',s{2});
    d_femur{count}=load([path,'SaveVariablesForPlotCol_.mat']);
    count=count+1;
end
...
for fi=[1,3,5]
    ...
    d_tibia{count}=load([path,'SaveVariablesForPlotClu_.mat']);
    count=count+1;
end
