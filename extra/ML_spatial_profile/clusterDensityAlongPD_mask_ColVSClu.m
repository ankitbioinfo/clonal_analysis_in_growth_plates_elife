...
RZ_HZ_height={[-1142,1346], [-1085,1468], [-1223,1184],[-1168, 1299],[-1268,1444],[-996,1286]};
...
for gi=6%2:length(allpath)
        ...
        s=strsplit(path,'Nuclei_and_CellsP40_');
         inputpath=strcat('../proximalTibia_DistalFemur/MakeListNucleiLabelled/',s{2});
        inputpath_mask=strcat('../proximalTibia_DistalFemur/MakeListClustersMask/',s{2});
        
        outputpath=strcat('./processedData/',s{2});
...
sarah_mask_id=load([inputpath,'ClusterMask_good.dat']);

[~,LCC]=readClusterFile([inputpath,'Cluster_good.dat']); 
...
for i=1:length(zone)-1
    for j=1:length(LCC)
         cluster= clusterCenter1(j,:);
             if ((cluster(2) >zone(i)) &  (cluster(2) <=zone(i+1)))
...
[AvgMeanFeatures,AvgStdFeatures,Xbin,Ybin,dataAvgMeanFeatures]=save_data_for_create_profile(LCC_all,Features,RZ,HZ,radius);
...
[AvgMeanFeatures,AvgStdFeatures,Xbin,Ybin,dataAvgMeanFeatures]=save_data_for_create_profile(LCC_col,Features,RZ,HZ,radius_col);
save([outputpath,'SaveVariablesForPlotCol_.mat'],   'AvgMeanFeatures','AvgStdFeatures','Xbin','Ybin','dataAvgMeanFeatures');
nanmean(AvgMeanFeatures{33})

[AvgMeanFeatures,AvgStdFeatures,Xbin,Ybin,dataAvgMeanFeatures]=save_data_for_create_profile(LCC_clu,Features,RZ,HZ,radius_clu);
save([outputpath,'SaveVariablesForPlotClu_.mat'],   'AvgMeanFeatures','AvgStdFeatures','Xbin','Ybin','dataAvgMeanFeatures');
nanmean(AvgMeanFeatures{33})



function [AvgMeanFeatures,AvgStdFeatures,Xbin,Ybin,dataAvgMeanFeatures]=save_data_for_create_profile(LCC,Features,RZ,HZ,radius)
    binsize=21;
